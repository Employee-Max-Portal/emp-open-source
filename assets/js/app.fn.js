(function($) {
    ...
    // New Shipment modal show
    $('#newShipment').on('click', function(){
        mfp_modal('#newShipmentModal');
    });
	
    // Fund Requisition modal show
    $('#fundRequisition').on('click', function(){
        mfp_modal('#fundRequisitionModal');
    });
	
    // Server Request modal show
    $('#serverRequest').on('click', function(){
        mfp_modal('#serverRequestModal');
    });
    ...
    $('#authentication_btn').on('click', function(){
        if(authenStatus == 0){
            // User is deactivated - show activate option
    ...
    $('#cb_authentication').on('click', function(){
        var password = $(this).parents('.form-group').find("[name='password']");
        if (authenStatus == 0) {
            // For deactivated users - checkbox means activate
            if (this.checked) {
                // Activating - password required
                password.prop('disabled', false);
                $('#disableReason').hide('slow');
            } else {
                // Not activating - password not required
                password.val('').prop('disabled', true);
            }
        } else {
            // For active users - checkbox means deactivate
            if (this.checked) {
                // Deactivating - no password needed
                password.val('').prop('disabled', true);
                $('#disableReason').show('slow');
            } else {
                // Not deactivating - password required for change
                password.prop('disabled', false);
                $('#disableReason').hide('slow');
            }
        }
...
// get fund requisition details
function getFundRequisitionDetails(id) {
    $.ajax({
        url: base_url + 'ajax/getFundRequisitionDetails',
        type: 'POST',
        data: {'id': id},
        dataType: "html",
        success: function (data) {
...
// get designation details
function getResponsibilityDetails(id) {
    $.ajax({
        url: base_url + 'ajax/responsibility_details',
        type: 'POST',
        data: {'id': id},
        dataType: "json",
        success: function (data) {
            $('#eresponsibility_id').val(data.id);
            $('#eresponsibility_title').val(data.title);
            $('#eresponsibility_description').val(data.description);
            mfp_modal('#modal');
        }
    });
}
...
function getRoleResponsibilityDetails(designation_id) {
    $.ajax({
        url: base_url + 'ajax/role_responsibility_details',
        type: 'POST',
        data: { designation_id: designation_id },
        dataType: 'json',
        success: function(data) {
            $('#edit_designation_id').val(designation_id);
            $('#edesignation_id').val(designation_id).trigger('change');
            
            // Uncheck all checkboxes first
            $('#edit_responsibilities input[type="checkbox"]').prop('checked', false);
            
            var container = $('#edit_responsibilities');
            var allItems = container.find('.sortable-item').detach();
            
            // First, append assigned responsibilities in database order
            for (var i = 0; i < data.length; i++) {
                var resp_id = data[i];
                var item = allItems.filter('[data-id="' + resp_id + '"]');
                if (item.length) {
                    item.find('input[type="checkbox"]').prop('checked', true);
                    container.append(item);
                    allItems = allItems.not(item); // Remove from remaining items
                }
            }
            
            // Then append remaining unassigned responsibilities
...
function fn_printElem(elem, html = false)
{
    ...
    frameDoc.document.write('<link rel="stylesheet" href="' + base_url + 'assets/css/style.css">');
