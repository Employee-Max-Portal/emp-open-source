...
(function($) {
	...
	// Additional fallback for modal close
	$(document).on("click", "button[type='button'].modal-dismiss, .modal-dismiss", function(e) {
		e.preventDefault();
		e.stopPropagation();
	...
	// Keyboard support for modal close (Escape key)
	$(document).on("keydown", function(e) {
		if (e.keyCode === 27 && $.magnificPopup.instance && $.magnificPopup.instance.isOpen) {
			$.magnificPopup.close();
		}
	});
...
function mfp_modal(data){
	$.magnificPopup.open({
		...
		callbacks: {
			open: function() {
				// Ensure close buttons work after modal opens
				$('.modal-dismiss').off('click.mfp').on('click.mfp', function(e) {
					e.preventDefault();
					e.stopPropagation();
					$.magnificPopup.close();
				});
