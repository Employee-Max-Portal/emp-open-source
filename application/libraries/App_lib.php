...
/* 
	...
	function upload_system_logo($role)
	{
		...
		if (isset($_FILES["system_logo"]) && !empty($_FILES['system_logo']['name'])) {
			...
			if ($this->CI->upload->do_upload("system_logo")) {
				...
				log_message('error', 'Logo upload failed: ' . $this->CI->upload->display_errors());
				return false;
			}
		}else{
    ...
    {
        if (empty($branch_id)) {
            $array = array('' => translate('select_business_first'));
        } else {
	...
	public function getSelectList($table, $all = '')
    {
        ...
        $result = $this->CI->db->get($table)->result();
		 foreach ($result as $row) {
			$arrayData[$row->id] = isset($row->name) ? $row->name : (isset($row->title) ? $row->title : '');
		}

        return $arrayData;
    }
	
	public function getSelectList_v2($table, $all = '', $where = [])
	{
		$arrayData = ['' => translate('select')];

		// Optionally include "All" selector
		if ($all === 'all') {
			$arrayData['all'] = translate('all_select');
		}
		...
		// Apply optional WHERE conditions (like status, department, etc.)
		if (!empty($where) && is_array($where)) {
			$this->CI->db->where($where);
		}

		// Prefer predictable ordering
		if ($this->CI->db->field_exists('title', $table)) {
			$this->CI->db->order_by('title', 'ASC');
		} elseif ($this->CI->db->field_exists('name', $table)) {
			$this->CI->db->order_by('name', 'ASC');
		}

		$result = $this->CI->db->get()->result();

		// Build dropdown list (supporting both title and name fields)
		foreach ($result as $row) {
			$label = '';
			if (isset($row->name)) {
				$label = $row->name;
