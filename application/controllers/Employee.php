...
class Employee extends Admin_Controller
{
    ...
    public function unique_designation($name)
    {
	   ...
	   /* responsibility form validation rules */
	/* protected function responsibility_validation()
    {
        $this->form_validation->set_rules('title', translate('title'), 'trim|required');
        
    }
	// employee responsibilities user interface and information are controlled here
    public function responsibilities()
    {
        if ($_POST) {
            if (!get_permission('responsibilities', 'is_add')) {
                access_denied();
            }
            $this->responsibility_validation();
            if ($this->form_validation->run() !== false) {
                $arrayResponsibility = array(
                    'title' => $this->input->post('title'), 
                    //'description' => $this->input->post('description'), 
                );
                $this->db->insert('staff_responsibilities', $arrayResponsibility);
                set_alert('success', translate('information_has_been_saved_successfully'));
                redirect(base_url('employee/responsibilities'));
            }
        }
        ...
        $this->data['title'] = translate('employee_responsibilities');
        $this->data['sub_page'] = 'employee/responsibilities';
        $this->data['main_menu'] = 'employee';
        $this->load->view('layout/index', $this->data);
    } */
    
    // Add responsibility with role assignment
    /* public function add_responsibility_with_role()
    {
        if (!get_permission('responsibilities', 'is_add')) {
            access_denied();
        }
        ...
        if ($this->form_validation->run() !== false) {
            ...
            $this->db->insert('staff_responsibilities', array('title' => $title));
            $responsibility_id = $this->db->insert_id();
            
            // Get current max priority for this designation
            if (empty($priority)) {
                $this->db->select_max('priority');
                $this->db->where('staff_designation', $designation_id);
                $result = $this->db->get('staff_roles_responsibilities')->row();
                $priority = ($result->priority ?? 0) + 1;
            } else {
                // Shift existing priorities
                $this->db->set('priority', 'priority + 1', FALSE);
                $this->db->where('staff_designation', $designation_id);
                $this->db->where('priority >=', $priority);
                $this->db->update('staff_roles_responsibilities');
            }
            
            // Insert role responsibility mapping
    ...
    // Delete single role responsibility
   /*  public function delete_single_role_responsibility($designation_id = '', $responsibility_id = '')
    {
        if (!get_permission('responsibilities', 'is_delete')) {
            access_denied();
        }
    ...
    /* public function edit_role_responsibility($designation_id = '', $responsibility_id = '')
    {
        ...
        if ($_POST) {
            $this->form_validation->set_rules('title', translate('title'), 'trim|required');
            $this->form_validation->set_rules('priority', translate('priority'), 'trim|numeric');
            
            if ($this->form_validation->run() !== false) {
                $title = $this->input->post('title');
                $priority = $this->input->post('priority');
                
                // Update responsibility title
                $this->db->where('id', $responsibility_id);
                $this->db->update('staff_responsibilities', array('title' => $title));
                
                // Update priority if provided
                if (!empty($priority)) {
                    $this->db->where('staff_designation', $designation_id);
                    $this->db->where('responsibility_id', $responsibility_id);
                    $this->db->update('staff_roles_responsibilities', array('priority' => $priority));
                }
        ...
        // Get current data
        $this->db->select('sr.title, srr.priority');
        ...
        $current_data = $this->db->get()->row_array();
        
        $this->data['current_data'] = $current_data;
        $this->data['designation_id'] = $designation_id;
        $this->data['responsibility_id'] = $responsibility_id;
        $this->data['designations'] = $this->db->get('staff_designation')->result_array();
        $this->data['title'] = translate('edit_responsibility');
        $this->data['sub_page'] = 'employee/edit_responsibility';
        $this->data['main_menu'] = 'employee';
        $this->load->view('layout/index', $this->data);
    } */

   /*  public function responsibilities_edit()
    {
        if (!get_permission('responsibilities', 'is_edit')) {
            ajax_access_denied();
        }
        
        // Set validation rules for edit form
        $this->form_validation->set_rules('responsibility_title', translate('title'), 'trim|required');
                
		if ($this->form_validation->run() !== false) {
			$arrayResponsibility = array(
					'title' => $this->input->post('responsibility_title'), 
				);
            $responsibility_id = $this->input->post('responsibility_id');
            $this->db->where('id', $responsibility_id);
            $this->db->update('staff_responsibilities', $arrayResponsibility);
            set_alert('success', translate('information_has_been_updated_successfully'));
            $array  = array('status' => 'success');
        } else {
            $error = $this->form_validation->error_array();
    ...
    public function responsibilities_delete($id)
    {
        if (!get_permission('responsibilities', 'is_delete')) {
            access_denied();
        }
        $this->db->where('id', $id);
        $this->db->delete('staff_responsibilities');
    }
    ...
    // Get role responsibility data for modal edit
    public function get_role_responsibility_data()
    {
        if (!get_permission('responsibilities', 'is_edit')) {
        ...
        $result = $this->employee_model->get_role_responsibility_details($designation_id, $responsibility_id);
        echo json_encode($result);
    }
    
    // Update role responsibility via AJAX
    public function update_role_responsibility()
    {
        if (!get_permission('responsibilities', 'is_edit')) {
            echo json_encode(array('status' => 'error', 'message' => 'Access denied'));
            return;
        ...
        // Basic validation
        if (empty($title)) {
            echo json_encode(array('status' => 'error', 'errors' => array('title' => 'Title is required')));
            return;
        }
        
        if (!empty($priority) && !is_numeric($priority)) {
            echo json_encode(array('status' => 'error', 'errors' => array('priority' => 'Priority must be numeric')));
            return;
        }
        
        // Update responsibility title
        $this->db->where('id', $responsibility_id);
        $updated = $this->db->update('staff_responsibilities', array('title' => $title));
        ...
        if ($updated) {
            ...
            set_alert('error', translate('Update failed'));
        }
