    ...
    <div class="col-md-9" style="height: 100%; overflow-y: auto;">
					...
					<div class="modal-body">
						...
						<div class="modal-sidebar">
							...
							<div class="sidebar-item">
								...
								<input type="date" id="modalDueDateInput" class="form-control" disabled>
							</div>
							
							<!-- Time Tracking -->
							<div class="sidebar-item">
								<label><i class="fas fa-clock"></i> <?= translate('estimation') ?></label>
								<input type="number" step="0.1" id="modalEstimationInput" class="form-control" placeholder="Hours" disabled>
							</div>
							<div class="sidebar-item">
		...
		function showToast(message, type = 'success') {
			...
			function populateModal(task, taskId) {
				...
				categorySelect.value = task.category || 'Milestone';
				
				// Setup dynamic milestone loading for modal category changes
				categorySelect.addEventListener('change', function() {
					const selectedCategory = this.value;
					const milestoneSelect = document.getElementById('modalMilestoneSelect');
					
					if (selectedCategory === 'EMP Request' || selectedCategory === 'Incident') {
						loadModalAllMilestones(task.milestone);
					} else {
						loadModalInProgressMilestones(task.milestone);
					}
				});
				
				// Load appropriate milestones based on current category
				if (task.category === 'EMP Request' || task.category === 'Incident') {
					loadModalAllMilestones(task.milestone);
				} else {
					loadModalInProgressMilestones(task.milestone);
				}
				
				// Functions to load milestone options for modal
				function loadModalAllMilestones(selectedMilestone = null) {
					$.ajax({
						url: '<?= base_url("tracker/get_all_milestones") ?>',
						type: 'POST',
						dataType: 'json',
						success: function(data) {
							...
							$.each(data, function(key, value) {
								milestoneSelect.innerHTML += '<option value="'+ key +'"'+ (key == selectedMilestone ? ' selected' : '') +'>'+ value +'</option>';
							});
						}
					});
				}
				
				function loadModalInProgressMilestones(selectedMilestone = null) {
					$.ajax({
						url: '<?= base_url("tracker/get_in_progress_milestones") ?>',
						type: 'POST',
						dataType: 'json',
			...
			// Add event listeners for status, priority, and category changes
			document.querySelectorAll('#modalStatusSelect, #modalPrioritySelect, #modalCategorySelect, #modalParentTaskSelect, #modalTaskTypeSelect').forEach(select => {
				...
				select.addEventListener('change', function() {
					...
					const fieldMap = {'status': 'task_status', 'priority': 'priority_level', 'category': 'category'};
			...
			<script>
					...
					function loadIssues(loadAll = false) {
						...
						const url = `<?= base_url('tracker/get_my_issues_data') ?>${loadAll ? '?load_all=true' : ''}`;
