        ...
        <div class="issue-tracker">
            ...
            <div id="loadingState" class="panel-body">
                <div style="text-align: center; padding: 60px 20px; color: #666;">
                    ...
                    <p>Please wait while we fetch all issues</p>
                </div>
            </div>
           ...
           <!-- Task Details Modal -->
			...
			function openSubIssueModal(parentId) {
				// Get parent task milestone from the modal (milestone is displayed as text, not select)
			...
			$(document).on('submit', '#subTaskModal form', function(e) {
				e.preventDefault();
				const form = $(this);
				...
				$.ajax({
					...
					success: function(response) {
						if(response.success) {
							...
							showToast('Sub-issue created successfully');
							form[0].reset(); // Reset form
						} else {
							showToast(response.message || 'Failed to create sub-issue', 'error');
						}
					},
					error: function() {
		...
		function showToast(message, type = 'success') {
			...
			<script>
					...
					function loadIssues(loadAll = false) {
						...
						const url = `<?= base_url('tracker/get_all_issues_data/') ?>${loadAll ? '?load_all=true' : ''}`;
