...
<div class="row" style="height: calc(108vh);">
    ...
    <div class="col-md-9" style="height: 100%; overflow-y: auto;">
		...
		<div class="panel">
			<header class="panel-heading d-flex justify-content-between align-items-center">
				<h4 class="panel-title"><?= translate('initiatives') ?></h4>
				<?php if (get_permission('tracker_initiatives', 'is_add')): ?>
				<div class="panel-btn">
					<a href="javascript:void(0);" class="btn btn-sm btn-primary" onclick="getInitiativesAddModal()">
    ...
    <div class="panel-body">
        ...
        foreach ($initiatives as $initiative) {
            $grouped[$initiative->lead_id][] = $initiative;
        }
        if (!empty($grouped)):
                ...
                <div class="accordion-body" id="accordion-body-<?= $uniqueId ?>">
                    ...
                    <div class="component-item">
                        ...
                        <div class="component-actions">
                            <a href="javascript:void(0);" class="btn btn-default btn-circle icon" onclick="getInitiativesEdit(<?= $row->id ?>)">
                                <i class="fas fa-pen-nib"></i>
                            </a>
                            <?= btn_delete('tracker/delete_initiatives/' . $row->id); ?>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
    ...
    <script>
        ...
        function getInitiativesEdit(id) {
            // Your existing edit functionality
            console.log('Edit initiative:', id);
        }
		...
		<!-- Edit initiatives Modal -->
		<div class="zoom-anim-dialog modal-block modal-block-primary mfp-hide" id="modal">
			<section class="panel" id="quick_view"></section>
		</div>

		<!-- Add initiatives Modal -->
		<div id="initiativeAddModal" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
			<section class="panel">
				<header class="panel-heading">
					<h4 class="panel-title"><i class="fas fa-plus-circle"></i> <?= translate('add_initiatives') ?></h4>
				</header>
				<?php echo form_open('tracker/add_initiatives', ['class' => 'form-horizontal']); ?>
				<div class="panel-body">
					...
					<div class="row">
						<div class="col-md-12 text-right">
							<button type="submit" class="btn btn-default mr-xs" data-loading-text="<i class='fas fa-spinner fa-spin'></i> Processing">
								<i class="fas fa-plus-circle"></i> <?= translate('add_initiatives') ?>
							</button>
...
function getInitiativesEdit(id) {
    $.ajax({
        url: base_url + 'tracker/get_initiatives_edit',
        type: 'POST',
        data: { id: id },
...
function getInitiativesAddModal() {
    mfp_modal('#initiativeAddModal');
}
</script>
