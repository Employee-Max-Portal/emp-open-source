        ...
        <?php foreach ($sops as $index => $sop): ?>
            <div class="sop-item">
                ...
                <div class="mb-md">
                    ...
                    <button type="button" class="btn btn-default btn-circle icon" data-toggle="modal" data-target="#executorModal<?= $sop['id'] ?>"><i class="fas fa-eye"></i></button>
                </div>

                <!-- Verifier Flow -->
                <div class="mb-md">
                    <label class="text-bold">Verifier Flow:</label>
                    <button type="button" class="btn btn-default btn-circle icon" data-toggle="modal" data-target="#verifierModal<?= $sop['id'] ?>"><i class="fas fa-eye"></i></button>
                </div>
            ...
            <!-- Executor Flow Modal -->
            <div class="modal fade" id="executorModal<?= $sop['id'] ?>" tabindex="-1" role="dialog">
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Executor Flow - <?= html_escape($sop['title']) ?></h5>
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                        </div>
                        <div class="modal-body text-center">
                            <?php if ($sop['executor_mermaid']) { ?>
                                <img src="<?= mermaidImageUrl($sop['executor_mermaid']) ?>" alt="Executor Flow" style="max-width: 100%;">
                            <?php } else { ?>
            ...
            <!-- Verifier Flow Modal -->
            <div class="modal fade" id="verifierModal<?= $sop['id'] ?>" tabindex="-1" role="dialog">
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Verifier Flow - <?= html_escape($sop['title']) ?></h5>
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                        </div>
                        <div class="modal-body text-center">
                            <?php if ($sop['verifier_mermaid']) { ?>
                                <img src="<?= mermaidImageUrl($sop['verifier_mermaid']) ?>" alt="Verifier Flow" style="max-width: 100%;">
                            <?php } else { ?>
